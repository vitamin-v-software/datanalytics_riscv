...
def build_model() -> [nn.Module, nn.Module]:
    resnet_model = models.resnet50(num_classes=config.model_num_classes)
    resnet_model = resnet_model.to(device=device, memory_format=torch.channels_last)
    
    # Load pretrained weights (excluding fc layer due to class mismatch)
    checkpoint = torch.load('./models/resnet50_checkpoint.pth', map_location=device)
    state_dict = checkpoint['state_dict'] if 'state_dict' in checkpoint else checkpoint
    
    # Remove fc layer from checkpoint
...
def define_optimizer(models) -> optim.SGD:
...
def train(
    ...
    while batch_data is not None:
        ...
        # Update EMA
        ema_model.update_parameters(models)
